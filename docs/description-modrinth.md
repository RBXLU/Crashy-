...
> ⚠️ **Sable is required.** Crashy! does nothing on its own. Install Sable (and its own
...
**Physics Activator** — turns a glued build into a live physics object. Right-click it with the
*item* version on an existing object to switch physics back off: the build snaps to the grid,
settles into the world, and stays glued so you can activate it again.

**Launcher** — grab, charge, fire. Power 1–10, roughly 8 m/s per level by default, so a full charge
throws at about 80 m/s.

**Build Saver** — a drafting table that records the build attached to it. Right-click it with a
**Blueprint** to copy the structure onto the blueprint, then stamp that build out anywhere, already
glued. Blueprints are reusable and never overwrite blocks that are already there.
...
| **Realistic** | Impact force is measured against each block's blast resistance. A power-8 hit scatters planks across the whole crater but only chips cobblestone near the middle. Obsidian shrugs it off entirely. |
...
- Minecraft **1.21.1**
- **NeoForge 21.1.0+**
- **[Sable](https://modrinth.com/mod/sable) 2.0.0+** and its dependencies (Veil, Forge Config API
  Port)
...
Crashy! — это набор инструментов для выживания поверх [**Sable**](https://modrinth.com/mod/sable),
библиотеки физики подвижных блочных структур. Sable даёт настоящую физику твёрдых тел, но намеренно
не даёт способа пользоваться ей в игре. Crashy! — и есть этот способ.
...
3. **Запусти.** ПКМ **Запускателем** по объекту — он зависает перед тобой. Зажми ПКМ для зарядки
   до **силы 10**, отпусти — и полетело.
...
- **Активатор физики** — включает физику. Тем же предметом можно **выключить** её обратно: постройка
  впишется в сетку мира и останется склеенной.
- **Запускатель** — взять, зарядить, выстрелить. Сила 1–10, по умолчанию ~8 м/с за единицу.
- **Сохранитель построек** и **Чертёж** — записывают постройку и вставляют её где угодно, уже
  склеенной. Чертёж многоразовый и не затирает чужие блоки.
...
| **Реалистичное** | сила удара сравнивается со взрывоустойчивостью блока: при силе 8 доски разлетаются по всей воронке, булыжник крошится только в середине, обсидиан не трогается |
...
Minecraft **1.21.1**, **NeoForge 21.1.0+**, **[Sable](https://modrinth.com/mod/sable) 2.0.0+** с его
зависимостями (Veil, Forge Config API Port).
